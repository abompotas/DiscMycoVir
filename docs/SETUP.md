...
3. Create the persistent volumes:
   ...
   **[--opt](https://docs.docker.com/reference/cli/docker/volume/create/#opt)** 
   parameter of the ```docker volume create``` command:
    ...
    ```
   **Optional:** If you want to run BLASTn using a local database, it is advised to store this database in

...
from pocketome.model import PocketomeJob
from shared import config

pocketome_blueprint = Blueprint('pocketome_blueprint', __name__)


@pocketome_blueprint.route('/pocketome', methods=['POST'])
def submit_pocketome_job():
...
@pocketome_blueprint.route('/pocketome/<int:job_id>/<string:job_hash>', methods=['GET'])
def pocketome_job_results(job_id=0, job_hash=None):
...
@pocketome_blueprint.route('/pocketome/<int:job_id>/<string:job_hash>/download', methods=['GET'])
def pocketome_job_results_zipped(job_id=0, job_hash=None):
    if job_id != 0 and job_hash is not None:
        job = PocketomeJob.get(job_id)
        if job is not None:
            if job.verify_hash(job_hash):

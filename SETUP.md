# MycoVirus Discovery - Setup Guide
...
3. Create the persistent volumes:
    ...
    ```
   __Optional:__ For performance reasons it is advised to run the app in machine with SSD storage. 
   ...
   parameter of the ```docker volume create``` command:
    ...
    ```
   __Optional:__ If you want to run BLASTn using a local database, it is advised to store this database in
   one of the persistent volumes (run this after you have created the containers):
    ```shell
    docker cp /host/path/to/db discvir.analyser:/virus-discovery/uploads/db
    ```
